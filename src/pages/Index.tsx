...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-[#1a0b2e] to-black relative overflow-hidden">
      ...
      <div className="absolute top-20 left-10 w-32 h-32 bg-primary/20 rounded-full blur-3xl float opacity-50" />
      <div className="absolute bottom-40 right-10 w-40 h-40 bg-pink-500/15 rounded-full blur-3xl float-delayed opacity-50" />
      <div className="absolute top-1/2 left-1/4 w-24 h-24 bg-blue-500/10 rounded-full blur-2xl float-delayed opacity-50" />
      ...
      {user && (
        ...
        >
          <Link to="/editor">
            <Button
              size="sm"
              className="bg-white/10 backdrop-blur-md border border-white/20 hover:bg-white/20 text-foreground gap-2"
            >
      ...
      <div className="relative max-w-md mx-auto px-6 py-12 flex flex-col gap-8">
        ...
        >
          ...
          {!user && (
            <Link 
              to="/auth" 
              className="text-xs text-muted-foreground/40 hover:text-muted-foreground/60 transition-colors mt-2 inline-block"
            >
              Admin Login
            </Link>
          )}
